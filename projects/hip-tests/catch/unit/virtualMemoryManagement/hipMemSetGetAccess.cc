...
HIP_TEST_CASE(Unit_hipMemSetAccess_MultDevSetGet) {
  ...
  if (device_count < 2) {
    HIP_SKIP_TEST("Need 2 GPUs to run test. Skipping Test..");
  }
...
HIP_TEST_CASE(Unit_hipMemSetAccess_FuncTstOnMultDev) {
  ...
  HIP_CHECK(hipGetDeviceCount(&devicecount));
  if (devicecount < 2) {
    HIP_SKIP_TEST("Machine is Single GPU. Skipping Test..");
  }
...
HIP_TEST_CASE(Unit_hipMemSetAccess_Vmm2UnifiedMemCpy) {
  ...
  if (managed != 1) {
    HIP_SKIP_TEST("GPU doesn't support managed memory.Skipping Test..");
  }
  size_t granularity = 0;
  constexpr int N = DATA_SIZE;
  size_t buffer_size = N * sizeof(int);
