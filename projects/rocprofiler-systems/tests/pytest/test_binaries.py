...
def flatten_json_keys(obj: dict | list | str | int | float | bool,
                      prefix: str = "") -> set[str]:
...
def validate_format_consistency(txt_vars: set[str],
                                json_paths: set[str]) -> list[str]:
    ...
    for var in sorted(txt_vars):
        ...
        if var in ENV_VAR_TO_JSON_PATH:
            ...
            if not any(json_key in path for path in json_paths):
                missing.append(
                    f"{var} -> expected JSON path '{json_key}' not found"
                )
        else:
